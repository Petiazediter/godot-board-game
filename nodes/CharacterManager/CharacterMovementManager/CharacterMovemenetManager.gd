...
@onready var action_manager: ActionManager = get_parent();
...
func _on_movement_ends() -> void:
	if action_manager.character_manager.action_in_progress == self:
		action_manager.character_manager.stop_current_action();
		action_manager.character_manager.board.update();
...
func _physics_process(_delta: float) -> void:
	if !action_manager.character_manager.get_is_action_in_progress():
...
func update_grid_visibility():
	var char_id: int = action_manager.character_manager.id_selected_character;
	if (char_id < 0 or !last_position):
		board.astar_grid.draw_path([],[]);
	else:
		var player: PlayableCharacter = action_manager.character_manager.get_selected_character();
...
func move_to_grid():
	if !last_position or action_manager.character_manager.get_is_action_in_progress(): return;
	var player_id: int = action_manager.character_manager.id_selected_character;
	...
	if board.astar.has_point(p_id):
		...
		action_manager.character_manager.set_action_in_progress(self);
		move_selected_character(astar_path.paths);

func move_selected_character(path: Array[Vector2]) -> void:
	var selected_character: PlayableCharacter = action_manager.character_manager.get_selected_character();
